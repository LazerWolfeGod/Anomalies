...
var structures = {
	...
	"Corruption":[{"Cost":{"Oxygen":4},"Upkeep":{"Blood_Pressure":1},"Output":{"Oxygen":1,"Max_Blood_Pressure":3}},
				{"Cost":{"Oxygen":8},"Upkeep":{"Blood_Pressure":3},"Output":{"Oxygen":2,"Max_Blood_Pressure":10}},
				{"Cost":{"Oxygen":20},"Upkeep":{"Blood_Pressure":8},"Output":{"Oxygen":4,"Max_Blood_Pressure":20}}]
}
const struct_owners = {
	...
	for y in range(tilemap_rect.size.y):
		...
		for x in range(tilemap_rect.size.x):
			...
			if tileinfo[-1][-1]["Structures"].size()>0 and tileinfo[-1][-1]["Structures"][0]["Owner"] == corruption.Id:
				corruption.set_core(tileinfo[-1][-1]["Map_Pos"])
...
func _process(delta):
	...
	for c in factions["Corruptions"]:
		var growto = c.gametick(tileinfo)
...
func add_structure(pos,structure):
	tileinfo[pos.y][pos.x]["Structures"].append(structure)
