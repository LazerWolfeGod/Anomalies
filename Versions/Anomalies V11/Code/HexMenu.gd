...
func ready(d):
	...
	if not tile_accessible and not tile_owned:
		...
		if tile_owned:
			...
			$Buildable/Owned/Hex_Label.text = data["Tile"]+" Tile"
		else:
...
func refresh_current_list():
	...
	for y in range(data["Structures"].size()):
		$Buildable/Owned/Structure_List.set_cell(0,Vector2i(fmod(y,2),int(y/2)),0,atlas_dict[data["Structures"][y]["Name"]])
		var xpos = 0
		for mat in data["Structures"][y]["Cost"]:
			make_sprite(mat,xpos,y,data["Structures"][y]["Cost"][mat])
			xpos+=1
