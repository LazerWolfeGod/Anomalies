...
var research = {
	...
	"Cardiac_Strength":{"Level":0,"info":{"Costs":[],"Modifers":[5,30,40,50,60,70,80,90,100,110,120]}},
...
func _init():
	for r in research:
		research[r]["Modifer"] = research[r]["info"]["Modifers"][-1]
		
func set_core(core):
	coretile = core
func set_struc(struc):
	structures = struc
...
func refresh_owned_tiles(tileinfo):
	...
	for y in range(tileinfo.size()):
		for x in range(tileinfo[y].size()):
			if not(tileinfo[y][x]["Structure"] is String) and tileinfo[y][x]["Structure"]["Owner"] == Id:
	...
	for y in range(tileinfo.size()):
		for x in range(tileinfo[y].size()):
			if not tileinfo[y][x]["Structure"] is String:
				var struc = tileinfo[y][x]["Structure"]
...
func gametick(tileinfo,delta):
	...
	if not queue_expansion["Waiting"]:
		...
		if pullblood_togrow(queue_expansion["Tile"],2):
			queue_expansion["Waiting"] = false
			resources["Oxygen"]["Amount"]-=queue_expansion["Cost"]
			#print("Built at ",queue_expansion["Tile"]["Grid_Pos"], " For ",queue_expansion["Cost"])
			refresh(tileinfo)
			var building = "Corruption"
			if not(queue_expansion["Tile"]["Structure"] is String): building = "Upgrade"
			return [queue_expansion["Tile"],building]
		else:
			queue_expansion["Waiting"] = false
	return 0

func pullblood_togrow(tile,blood_needed):
	var around = [tile["Grid_Pos"]]
...
func bloodflow(tileinfo):
	...
	for y in range(tileinfo.size()):
		for x in range(tileinfo[y].size()):
			...
			if not tile["Structure"] is String and tile["Structure"]["Owner"] == Id:
				var core = false
				...
				for a in range(around.size()):
					...
					if around[a].x>-1 and around[a].x<tileinfo[y].size() and around[a].y>-1 and around[a].y<tileinfo.size():
						...
						if adjacent["Owner"] == Id:
							...
							if blood_to_move<0: blood_to_move = 0
							#print("At ",tile["Grid_Pos"] ," Moved ",blood_to_move," out of ",tile["Blood_Pressure"]["Amount"])
...
func grow(tileinfo):
	...
	for y in range(tileinfo.size()):
		for x in range(tileinfo[y].size()):
			if (tileinfo[y][x]["Owner"] == "None" and tileinfo[y][x]["AccessibleTo"][Id] and tileinfo[y][x]["Tile"] != "Water") or (tileinfo[y][x]["Owner"] == Id and tileinfo[y][x]["Structure"]["Level"]<structures[tileinfo[y][x]["Structure"]["Name"]].size()):
		...
		if s["Active"]:
			for u in s["Upkeep"]:
				if u == "Blood_Pressure":
					...
					#print(s["Name"]," took ",s["Upkeep"][u], " ", u)
					resources[u]["Amount"]-=s["Upkeep"][u]
			for u in s["Output"]:
				if u.split("_")[0] != "Max":
					#print(s["Name"]," gave ",s["Output"][u], " ", u)
...
func refresh_owned_structures(tileinfo):
	...
	for y in tileinfo:
		for x in y:
			if not x["Structure"] is String:
				if x["Structure"]["Owner"] == Id:
					owned_structures.append(x["Structure"])
