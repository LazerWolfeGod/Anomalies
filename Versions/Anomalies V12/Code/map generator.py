...
def poisson_disk_sampling(w,h,r,k=30):
    ...
    output = []
    grid = [[[] for x in range(w//r+1)] for y in range(h//r+1)]
...
x = []
y = []
for a in range(10,200,2):
    start = time.perf_counter()
    points = poisson_disk_sampling(2000,1200,a)
##    plt.plot([a,time.perf_counter()-start])
    x.append(len(points))
...
##    draw_grid(grid,30)
    for a in points:
        pygame.draw.circle(screen,(0,0,255),a,2)
    ui.rendergui(screen)
    pygame.display.flip()
    clock.tick(60)                                               
pygame.quit() 
